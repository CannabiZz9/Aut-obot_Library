...
class Config:
    ...
    # GPIO
    CHIP_PATH = "/dev/gpiochip0"  # Adjust if your Pi 5 maps this differently
    LED_WIFI = 17
    LED_STATUS = 27
    WIFI_POLL_S = 2.0
...
class GPIOHandler:
    def __init__(self):
        self.request = None
        self.pins = {
            "wifi": Config.LED_WIFI,
            "status": Config.LED_STATUS
        }
        try:
            # Modern gpiod v2 approach: Request all lines simultaneously
            self.request = gpiod.request_lines(
                Config.CHIP_PATH,
                consumer="bot",
                config={
                    Config.LED_WIFI: gpiod.LineSettings(direction=Direction.OUTPUT, output_value=Value.INACTIVE),
                    Config.LED_STATUS: gpiod.LineSettings(direction=Direction.OUTPUT, output_value=Value.INACTIVE)
                }
            )
    ...
    def set(self, name, state):
        if self.request and name in self.pins:
            val = Value.ACTIVE if state else Value.INACTIVE
            self.request.set_value(self.pins[name], val)
